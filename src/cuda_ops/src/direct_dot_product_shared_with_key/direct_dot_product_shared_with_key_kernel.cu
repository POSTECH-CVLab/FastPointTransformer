...
#include "direct_dot_product_shared_with_key_kernel.h"


__global__ void direct_dot_product_shared_with_key_forward_kernel(
...
)
{
    ...
    int key_idx_ = kq_indices[map_idx];
...
__global__ void direct_dot_product_shared_with_key_backward_kernel(
...
void direct_dot_product_shared_with_key_forward_launcher(
    ...
    direct_dot_product_shared_with_key_forward_kernel<<<blocks, threads, 0, stream>>>(
...
void direct_dot_product_shared_with_key_backward_launcher(
    ...
    dim3 threads(THREADS_PER_BLOCK);
    direct_dot_product_shared_with_key_backward_kernel<<<blocks, threads, 0, stream>>>(
